...
export default async ({ mode, wranglerMode, inProdLikeMode, command, isSSRBuild, env, appBaseURL, appType, targetEnv, staticDefs, pkgUpdates }) => {
    ...
    return {
        ...
        // i.e., The `closeBundle` hook fires after writing to disk.
        ...
        closeBundle: {
            ...
            handler: async (error) => {
                 ...
                 *
                 * We only prune `./.npmignore`s when building in a prod-like mode, as it’s possible there are files
                 * being compiled by TypeScript that are needed for development; i.e., they need to exist in to be
                 ...
                 * because we intend to enforce our standards. For further details {@see https://o5p.me/MuskgW}.
                 */
                 ...
                 * TypeScript that are needed for development; i.e., they need to exist in dev mode in order to be
                 * capable of serving their intended purpose; e.g., dev-only utilities, runners, sandbox files, etc.
                 */
                ...
                if ('build' === command && fs.existsSync(path.resolve(u.distDir, './vite/' + (isSSRBuild ? 'ssr-' : '') + 'manifest.json'))) {
                    u.log($chalk.gray('Generating SHA-1-keyed ' + (isSSRBuild ? 'SSR ' : '') + 'manifest.'));
